...
При вводе «−1» с клавиатуры необходимо вывести всё содержимое хранилища (20 или менее элементов). При 
выводе элементы должны быть в том порядке, в котором они добавлялись.
...
int main()
{
    ...
    while (num != -2)
    {
        if (num == -1)
        {
            for (int i = curpos; i < db.size(); i++) std::cout << db[i] << " ";
            for (int i = 0; i < curpos; i++) std::cout << db[i] << " ";
        }
        else
        {
            db[curpos] = num;
